...
const TEMP_DIR_PREFIX = _path.resolve(_.get(process.env, 'FILE_UTIL_TEST_TEMP_DIR_PREFIX', '/tmp/file-util-test-'));
const TEMP_DIR_ROOT = _.get(process.env, 'FILE_UTIL_TEST_TEMP_DIR_ROOT');
...
TestUtil.walkFileTree = function (fileTree, callback, includeBasePath) {
    var visit = function (node) {
        callback(node, fs.stat({path: node.path}));
        _.each(node.files, visit);
    };

    if (includeBasePath) {
        callback(fileTree, fs.stat({path: fileTree.path}));
    }
